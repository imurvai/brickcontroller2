...
namespace BrickController2.UI.Services.Dialog
{
    public class DialogService : IDialogService, IDialogServerHost
    {
        ...
        public Task<ProgressDialogResult> ShowProgressDialogAsync(bool isDeterministic, Func<IProgressDialog, CancellationToken, Task> action, string title = null, string message = null, string cancelButtonText = null)
        {
            return _dialogServer?.ShowProgressDialogAsync(isDeterministic, action, title, message, cancelButtonText) ?? Task.FromResult(new ProgressDialogResult(false));
        }
